...
def emp_login():
    ...
    if (res[0] == tb2.get()):
        # messagebox.showinfo("login", "logged in")
        cur.execute('SELECT emp_id FROM customer where username = "' + uname + '";')
        res1 = cur.fetchone()
        emp_id = res1[0]
        # global cart_id
        # cur.execute('SELECT cart_id from cust_cart_rel where cust_id = ' + str(cust_id) + ';')
...
def add2cart():
    ...
    temp_cost = res[0]
    cur.execute(
        'INSERT INTO cart_item VALUES(' + item_id_tb.get() + ',' + str(cart_id) + ',' + add_qty_tb.get() + ',' + str(
            int(temp_cost)) + ');')
    conn.commit()
...
def show_cart():
    ...
    cur.execute(
        "select C.item_id, C.qty, C.cost from cart_item as C, cust_cart_rel as r where r.cart_id = C.cart_id and r.cust_id = '" + str(
            cust_id) + "';")
    res = cur.fetchall()
    ...
    back_from_cart.grid(row=4, column=1)
    ...
    convert = Button(fsc, text="Place order", command=convert2order)
...
def show_orders():
    ...
    cur.execute(
        'SELECT o.order_id, o.order_date, o.cost, o.order_status, o.payment_id FROM orders as o, cust_order_rel as rel where rel.cust_id=' + str(
            cust_id) + ' and rel.order_id = o.order_id;')
    res = cur.fetchall()
    m = 0
    for i in res:
        tree.insert('', 'end', text=str(m), values=i)
...
def show_order_det():
    ...
    cur.execute(
        'SELECT oi.item_id, oi.qty, oi.cost FROM ORDER_ITEM as oi, cust_order_rel as rel WHERE oi.order_id=' + str(
            temp) + ' and rel.order_id = oi.order_id and rel.cust_id=' + str(cust_id) + ';')
    res = cur.fetchall()
...
def convert2order():
    global cust_id
    cur.execute('call cart2ord(' + str(cust_id) + ');')
    conn.commit()
